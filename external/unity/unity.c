...
void UnityAssertEqualIntArray(UNITY_INTERNAL_PTR expected,
    ...
    while ((elements > 0) && (elements--)) {
        ...
        if (expect_val != actual_val) {
            if ((style & UNITY_DISPLAY_RANGE_UINT) && (length < (UNITY_INT_WIDTH /
...
void UnityAssertEqualString(const char *expected,
    ...
    } else { /* handle case of one pointers being null (if both null, tests should pass) */
        if (expected != actual) {
            Unity.CurrentTestFailed = 1;
        }
    }
...
/*-----------------------------------------------*/
/* If we have not defined our own tests runner, then include our default tests runner to make life easier */
...
{
    /* Check if this tests name matches the included tests pattern */
    int retval;
    ...
    /* Check if this tests name matches the excluded tests pattern */
