...
/* These functions are intended to be called before and after each tests.
 * If using unity directly, these will need to be provided for each tests
 * executable built. If you are using the tests runner generator and/or
 ...
 * entire tests suite.  suiteTearDown() is passed the number of tests that
 * failed, and its return value becomes the exit code of main(). If using
 * Unity directly, you're in charge of calling these if they are desired.
 * If using Ceedling or the tests runner generator, these will be called
 ...
 * to support complex tests loops, etc. Both are NOT built into Unity. Instead
 * the tests runner generator will create them. resetTest will run teardown and
 * setup again, verifying any end-of-tests needs between. verifyTest will only
 ...
 *     - define UNITY_DIFFERENTIATE_FINAL_FAIL to print FAILED (vs. FAIL) at tests end summary - for automated search for failure

 * Optimization
 *     - by default, line numbers are stored in unsigned shorts.  Define UNITY_LINE_TYPE with a different type if your files are huge
 *     - by default, tests and failure counters are unsigned shorts.  Define UNITY_COUNTER_TYPE with a different type if you want to save space or have more than 65535 Tests.
 ...
 *     - you'll want to define UNITY_USE_COMMAND_LINE_ARGS if you have the tests runner passing arguments to Unity
 ...
 * This method allows you to abort a tests immediately with a PASS state, ignoring the remainder of the tests. */
#define TEST_PASS()                                                                                TEST_ABORT()
#define TEST_PASS_MESSAGE(message)                                                                 do { UnityMessage((message), __LINE__); TEST_ABORT(); } while(0)

/* This macro does nothing, but it is useful for build tools (like Ceedling) to make use of this to figure out
 * which files should be linked to in order to perform a tests. Use it like TEST_FILE("sandwiches.c") */
#define TEST_FILE(a)
